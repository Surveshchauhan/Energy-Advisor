...
def get_weather_forecast(location: str, days: int = 3) -> Dict[str, Any]:
    ...
    # Validate input
    if not location or days < 1 or days > 7:
        return {
            "error": "Invalid input. Please provide a location and days between 1 and 7."
        }
    ...
    # Simulate hourly forecast for each day
    hourly = []
    for day in range(days):
        for hour in range(24):
            hourly.append({
                "hour": hour + day * 24,
                "temperature_c": round(random.uniform(14, 32), 1),
                "condition": random.choice(["sunny", "partly_cloudy", "cloudy", "rainy"]),
                "solar_irradiance": round(random.uniform(100, 800), 1),  # W/m²
...
def get_electricity_prices(date: str = None) -> Dict[str, Any]:
    ...
    base_rate = 0.12  # USD per kWh
    peak_multiplier = 1.5
    off_peak_multiplier = 0.8
    demand_charge_peak = 0.05  # USD per kWh
    demand_charge_off_peak = 0.0

    hourly_rates = []
    for hour in range(24):
        if 6 <= hour < 22:
            rate = round(base_rate * peak_multiplier, 3)
            period = "peak"
            demand_charge = demand_charge_peak
        else:
            rate = round(base_rate * off_peak_multiplier, 3)
            period = "off_peak"
            demand_charge = demand_charge_off_peak
    ...
    try:
        ...
        if not os.path.exists(os.path.join(persist_directory, "chroma.sqlite3")):
            ...
            for doc_path in ["data/documents/tip_device_best_practices.txt", "data/documents/tip_energy_savings.txt",\
                "data/documents/energy_storage_optimization_1.txt","data/documents/energy_storage_optimization_2.txt",\
                "data/documents/hvac_optimization_strategies_1.txt","data/documents/hvac_optimization_strategies_2txt",\
                "data/documents/renewable_energy_integration_1.txt","data/documents/renewable_energy_integration_2.txt",\
                "data/documents/smart_home_automation_tips_1.txt","data/documents/smart_home_automation_tips_2.txt",\
                "data/documents/seasonal_energy_management_1.txt","data/documents/seasonal_energy_management_2.txt"]:
